...
int FindMinPath(struct AVLTree *tree, TYPE *path)
{
        ...
        if (tree->cnt > 1){
               ...
               /* FIX ME */
               preorder(current, &min_cost, path, &path_len, canidate_path, &c_path_len, &sumDiff, &parent_value);
               ...
               TYPE *candidate_path, int *c_path_len, TYPE sumDiff, TYPE parent_value)
{
    ...
    preorder(node->left, min_cost, path, path_len, canidate_path, c_path_len, sumDiff, parent_value);
    preorder(node->right, min_cost, path, path_len, canidate_path, c_path_len, sumDiff, parent_value);
}
